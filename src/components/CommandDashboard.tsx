...
export function CommandDashboard({
  ...
  return (
    <div id="command-dashboard-view" className="space-y-4">
      {/* Top Quick Metrics Ribbon */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {/* Metric 1 */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-lg">
          <div className="text-[10px] font-mono text-slate-400 uppercase">Total Scanned</div>
          <div className="text-xl font-extrabold text-cyan-400 font-mono mt-0.5">
            {metrics.totalDetectionsToday.toLocaleString()}
          </div>
          <div className="text-[10px] text-slate-400">All Zambian corridors</div>
        </div>

        {/* Metric 2 */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-lg">
          <div className="text-[10px] font-mono text-slate-400 uppercase">Speeding Hits</div>
          <div className="text-xl font-extrabold text-rose-400 font-mono mt-0.5">
            {metrics.totalSpeedViolations}
          </div>
          <div className="text-[10px] text-slate-400">Above statutory limits</div>
        </div>

        {/* Metric 3 */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-lg">
          <div className="text-[10px] font-mono text-slate-400 uppercase">Hotlist / Stolen</div>
          <div className="text-xl font-extrabold text-amber-400 font-mono mt-0.5">
            {metrics.totalHotlistHits}
          </div>
          <div className="text-[10px] text-slate-400">ZP Interpol flag matches</div>
        </div>
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-lg">
          ...
          <div className="text-[10px] text-slate-400">Awaiting dispatch</div>
        </div>

        {/* Metric 5 */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-lg">
          <div className="text-[10px] font-mono text-slate-400 uppercase">Patrol Units</div>
          <div className="text-xl font-extrabold text-emerald-400 font-mono mt-0.5">
            {metrics.activePatrols} / {patrolUnits.length}
          </div>
          <div className="text-[10px] text-slate-400">On-duty field cruisers</div>
        </div>

        {/* Metric 6 */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-lg">
          <div className="text-[10px] font-mono text-slate-400 uppercase">Edge AI Nodes</div>
          <div className="text-xl font-extrabold text-teal-400 font-mono mt-0.5">
            {metrics.onlineCameras} / {metrics.totalCameras}
          </div>
          <div className="text-[10px] text-slate-400">{metrics.networkLatencyMs}ms edge ping</div>
        </div>
      </div>

      {/* Live Road Scene & Offence Cinema Promo/Selector Banner */}
      <div className="bg-gradient-to-r from-rose-950/40 via-slate-900/90 to-slate-900/90 border border-rose-900/40 rounded-xl p-3.5 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <button
          ...
          className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-xl text-xs font-bold font-mono transition flex items-center gap-1.5 shrink-0 shadow-lg shadow-rose-900/30"
        >
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-lg flex flex-wrap items-center justify-between gap-2.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-2.5 py-1 bg-slate-800 hover:bg-rose-900/60 text-slate-200 hover:text-rose-200 border border-slate-700 hover:border-rose-600 rounded-lg text-xs font-mono font-medium transition"
          >
            ⚡ Inject Speeding Car
          </button>
          ...
          <button
            ...
            className="px-2.5 py-1 bg-slate-800 hover:bg-amber-900/60 text-slate-200 hover:text-amber-200 border border-slate-700 hover:border-amber-600 rounded-lg text-xs font-mono font-medium transition"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 bg-slate-800 hover:bg-cyan-900/60 text-slate-200 hover:text-cyan-200 border border-slate-700 hover:border-cyan-600 rounded-lg text-xs font-mono font-medium transition"
          >
            🚧 Stalled Lorry
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
              ...
              {cameras
                ...
                .map((cam) => (
                  <div
                    ...
                    className="p-2.5 bg-slate-950 hover:bg-slate-800/80 border border-slate-800 hover:border-emerald-500/50 rounded-xl cursor-pointer transition space-y-1.5 group"
                  >
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-bold text-slate-200 truncate group-hover:text-emerald-400">
                        {cam.id}
                      </span>
                    ...
                    <div className="text-[11px] text-slate-400 truncate">{cam.road}</div>

                    <div className="text-[10px] font-mono text-cyan-400 flex items-center justify-between pt-1 border-t border-slate-800">
                      <span>Detections: {cam.detectedCountToday}</span>
                      <span className="text-rose-400">Vio: {cam.activeViolationsToday}</span>
                    </div>
                  </div>
                ))}
            </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-xl flex flex-col h-[570px]">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-3">
              ...
              <button
                ...
                className={`px-2 py-1 rounded text-[10px] font-mono font-medium border transition ${
                  filterCityOnly
            ...
            <div id="live-alert-feed-list" className="space-y-2.5 overflow-y-auto pr-1 flex-1">
                ...
                displayedViolations.map((v) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-sm bg-slate-900 px-2 py-0.5 rounded border border-slate-700 text-amber-300">
                            {v.plateNumber}
                          </span>
                          <span
                        ...
                        <span className="text-[11px] font-mono text-slate-400">{v.timestamp}</span>
                      </div>

                      {/* Velocity and Limit */}
                      <div className="grid grid-cols-2 gap-2 text-[11px] font-mono bg-slate-900/80 p-2 rounded-lg border border-slate-800/80">
                        <div>
                          <span className="text-slate-400 block text-[10px]">RADAR SPEED:</span>
                          <span
                            className={`font-bold ${
                        ...
                        <div>
                          ...
                          <span className="text-slate-200">{v.speedLimit} km/h</span>
                        </div>
                      </div>

                      {/* Road and City */}
                      <div className="text-[11px] text-slate-300 truncate">
                        📍 {v.road} • <span className="text-slate-400">{v.city}</span>
                      </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          {isSpeeding && (
                            <button
                              ...
                              className="px-2 py-0.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-[11px] font-medium transition flex items-center gap-1"
                            >
                          ...
                          {isHotlist && (
                            <button
                              ...
                              className="px-2 py-0.5 bg-rose-600 hover:bg-rose-500 text-white rounded text-[11px] font-bold transition flex items-center gap-1 animate-pulse"
                            >
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-xl flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs font-bold text-slate-200">Patrol Fleet Response Ready</div>
            ...
            <button
              onClick={() => onSelectView('dispatch')}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-medium transition border border-slate-700"
            >
              Open Dispatch
            </button>
          </div>
