...
export function IncidentDispatchView({
  ...
  return (
    <div id="incident-dispatch-center" className="space-y-4">
      {/* Top Banner */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono bg-rose-950 text-rose-300 border border-rose-800 px-3 py-1.5 rounded-lg flex items-center gap-1.5">
      ...
      {dispatchSuccessMsg && (
        <div className="bg-emerald-950 border border-emerald-700 text-emerald-200 p-3 rounded-xl text-xs flex items-center gap-2 shadow-lg animate-in fade-in">
          <Check className="w-4 h-4 text-emerald-400 shrink-0" />
          <span className="font-mono font-medium">{dispatchSuccessMsg}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-5 bg-slate-900/90 border border-slate-800 rounded-xl p-3 shadow-xl flex flex-col space-y-2.5 max-h-[700px]">
          <div className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center justify-between px-1">
            ...
            <span className="text-[10px] font-mono text-slate-400">PRIORITY SORTED</span>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-1">
                    <span className="font-bold text-slate-200 truncate">{inc.title}</span>
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] font-mono border font-medium ${getStatusBadge(
                        inc.status
                      )}`}
                    >
                      {inc.status}
                  ...
                  <div className="text-slate-400 flex items-center gap-1 text-[11px]">
                    ...
                    <span className="truncate">{inc.location} ({inc.city})</span>
                  </div>

                  <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 pt-1 border-t border-slate-800/60">
                    <span>{inc.timestamp}</span>
                    <span className="text-emerald-400">AI Confidence: {(inc.confidenceScore * 100).toFixed(0)}%</span>
                  </div>
                </div>
              );
            })}
          ...
          {selectedIncident ? (
            <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
                ...
                <div className="flex items-center gap-2">
                  <a
                    href={`https://www.google.com/maps/@${selectedIncident.lat},${selectedIncident.lng},16z/data=!5m1!1e1`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-mono font-medium flex items-center gap-1.5 transition border border-slate-700"
                    title="View Incident Location with Live Traffic on Google Maps"
                  >
                  ...
                  {linkedCamera && (
                    <button
                      ...
                      className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-medium flex items-center gap-1.5 transition border border-slate-700"
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="relative rounded-lg overflow-hidden bg-slate-950 border border-slate-800 aspect-video">
                  ...
                  />
                  <div className="absolute bottom-2 left-2 bg-slate-950/80 backdrop-blur text-[10px] font-mono text-amber-300 px-2 py-0.5 rounded border border-slate-700">
                ...
                <div className="bg-slate-950 p-3.5 rounded-lg border border-slate-800 text-xs font-mono space-y-2">
                  <div>
                    <span className="text-slate-400 block text-[10px]">ROAD CORRIDOR:</span>
                    <span className="text-slate-200 font-bold">{selectedIncident.road}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">DETECTION LOCATION:</span>
                    <span className="text-slate-200">{selectedIncident.location}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">SURVEILLANCE NODE:</span>
                    <span className="text-emerald-400">{selectedIncident.cameraId}</span>
                  </div>
                  <div>
              ...
              <div className="bg-slate-950/80 p-3 rounded-lg border border-slate-800 text-xs text-slate-300">
                <span className="text-[10px] font-mono text-slate-400 block font-bold uppercase mb-1">
              ...
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 space-y-2.5">
                <div className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400 font-mono">STATUS: {selectedIncident.status}</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 py-2 bg-emerald-700 hover:bg-emerald-600 disabled:opacity-50 disabled:pointer-events-none text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition shadow"
                  >
                  ...
                  <button
                    ...
                    className="py-2 px-4 bg-slate-800 hover:bg-rose-900/70 text-slate-300 hover:text-rose-200 rounded-lg text-xs font-medium flex items-center justify-center gap-1.5 transition border border-slate-700"
                  >
              ...
              <form onSubmit={handleDispatch} className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                <div className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-rose-400">
                    <Radio className="w-4 h-4" />
                    <span>Stage 2: Tactical Patrol Intercept & Dispatch</span>
                  </span>
                  <span className="text-[10px] text-slate-400 font-mono">POLICE / RTSA VHF CHANNEL</span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none focus:ring-1 focus:ring-rose-500 font-mono"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none focus:ring-1 focus:ring-rose-500"
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 disabled:pointer-events-none text-white rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition shadow-lg shadow-rose-900/30"
                >
